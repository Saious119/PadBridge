...
# --- permissions -----------------------------------------------------------
# The daemon needs write access to /dev/uinput; the daemon and the GUI both
# need read access to /dev/input/event* (usually the "input" group).
NEED_SUDO=0
...
echo
read -r -p "Enable and start the PadBridge background service now? [Y/n] " answer
if [ "${answer:-Y}" != "${answer#[Yy]}" ] || [ -z "${answer:-}" ]; then
